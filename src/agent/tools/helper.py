...
def is_within_doctor_schedule(
    ...
    if appointment_datetime < start_dt:
        raise ScheduleValidationError(
            f"Appointment starts too early. Doctor starts at {start_time.strftime('%H:%M')} WIB, "
    ...
    if appointment_end > end_dt:
        raise ScheduleValidationError(
            f"Appointment ends too late. Doctor ends at {end_time.strftime('%H:%M')} WIB, "
            f"but {duration_minutes}-minute appointment would end at {appointment_end.strftime('%H:%M')}"
        )
