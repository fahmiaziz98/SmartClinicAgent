...
class InputCreateAppointment(BaseModel):
    ...
    duration_minutes: int = Field(
        default=30, description="Duration appointment in minutes (default: 30 minutes)"
    )
    ...
    symptoms: Optional[str] = Field(
        default="", description="Patient Complaints or symptons"
    )
    notes: Optional[str] = Field(default="", description="Notes for the appointment")
    phone_number: Optional[str] = Field(
        default="", description="Phone number of patient"
    )
...
class InputUpdateAppointment(BaseModel):
    ...
    start_datetime: Optional[datetime] = Field(
        default=None, description="New start time for event (optional)"
    )
    end_datetime: Optional[datetime] = Field(
        default=None, description="New end time for event (optional)"
    )
    description: Optional[str] = Field(
        default=None, description="New description for event (optional)"
    )
    location: Optional[str] = Field(
        default=None, description="New location for the event (optional)"
    )
