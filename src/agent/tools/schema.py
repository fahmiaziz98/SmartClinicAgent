...
class InputGetDoctorSchedule(BaseModel):
    ...
    start_datetime: datetime = Field(
        description="Start date and time to filter events (format: YYYY-MM-DD HH:MM:SS)"
    )
    end_datetime: datetime = Field(
        description="End date and time to filter events (format: YYYY-MM-DD HH:MM:SS)"
    )
...
class InputGetEventById(BaseModel):
    ...
    event_id: str = Field(
        description="Unique ID of the event to retrieve details"
    )
...
class InputCreateAppointment(BaseModel):
    ...
    appointment_datetime: datetime = Field(
        description="Appointment time (format: YYYY-MM-DD HH:MM:SS)"
    )
    duration_minutes: int = Field(
        default=30,
        description="Duration appointment in minutes (default: 30 minutes)"
    )
    ...
    symptoms: Optional[str] = Field(
        default="",
        description="Patient Complaints or symptons"
    )
...
class InputUpdateAppointment(BaseModel):
    ...
    event_id: str = Field(
        description="Unique event ID to be updated"
    )
    patient_name: str = Field(
        description="New patient name or exiting"
    )
    patient_email: str = Field(
        description="New patient email or exiting"
    )
    ...
    start_datetime: Optional[datetime] = Field(
        default=None,
        description="New start time for event (optional)"
    )
    end_datetime: Optional[datetime] = Field(
        default=None,
        description="New end time for event (optional)"
    )
    description: Optional[str] = Field(
        default=None,
        description="New description for event (optional)"
    )
...
class InputCancelAppointment(BaseModel):
    ...
    event_id: str = Field(
        description="Unique ID of the event to be deleted"
    )
    reason: str = Field(
